...
task :pack, [:config] do |t, args|
	args.with_defaults(:config => :Release)
    Dir.chdir('BadMedicine.Dicom') do
        sh "nuget pack BadMedicine.Dicom.nuspec -Properties Configuration=#{args.config} -IncludeReferencedProjects -Symbols -Version #{$VERSION}#{$SUFFIX}"
        sh "nuget push HIC.BadMedicine.Dicom.#{$VERSION}#{$SUFFIX}.nupkg -Source https://api.nuget.org/v3/index.json -ApiKey #{NUGETKEY}"
    end
end

task :build_cli => :restorepackages do
	Dir.chdir("BadDicom/") do
	...
	sh "powershell.exe -nologo -noprofile -command \"& { Add-Type -A 'System.IO.Compression.FileSystem'; [IO.Compression.ZipFile]::CreateFromDirectory('BadDicom/Publish', 'BadDicom/baddicom-cli-win-x64.zip'); }\""
end
...
task :github do
	...
	uri = URI.parse('https://api.github.com/repos/HicServices/BadMedicine.Dicom/releases')
	body = { tag_name: "v#{version}", name: "BadMedicine.Dicom v#{version}", body: ENV['MESSAGE'] || "Compiled binary (for win-x64) of BadMedicine.Dicom v#{version}", target_commitish: branch, prerelease: prerelease }
	...
	Dir.chdir("BadDicom") do
		upload_to_github(upload_url, "baddicom-cli-win-x64.zip")
    end
end
